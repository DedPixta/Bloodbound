...
function bloodbound:OnGameRulesStateChange(keys)
	--PrintTable(keys)

	local new_state = GameRules:State_Get()

	if new_state == DOTA_GAMERULES_STATE_INIT then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_INIT")
	elseif new_state == DOTA_GAMERULES_STATE_WAIT_FOR_PLAYERS_TO_LOAD then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_WAIT_FOR_PLAYERS_TO_LOAD")
	elseif new_state == DOTA_GAMERULES_STATE_CUSTOM_GAME_SETUP then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_CUSTOM_GAME_SETUP")
	...
	elseif new_state == DOTA_GAMERULES_STATE_HERO_SELECTION then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_HERO_SELECTION")
		PreCacheConfig:postLoad()
		bloodbound:OnAllPlayersLoaded()
	elseif new_state == DOTA_GAMERULES_STATE_STRATEGY_TIME then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_STRATEGY_TIME")
		...
		for playerID = 0, DOTA_MAX_TEAM_PLAYERS - 1 do
			if PlayerResource:IsValidPlayerID(playerID) and PlayerResource:IsValidPlayer(playerID) then
				...
				if not PlayerResource:HasSelectedHero(playerID) and PlayerResource:IsConnected(playerID) then
					...
					DebugPrint("[DEBUG] Randomed a hero for a player number " .. playerID)
				end
			end
		end
	elseif new_state == DOTA_GAMERULES_STATE_TEAM_SHOWCASE then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_TEAM_SHOWCASE")
	elseif new_state == DOTA_GAMERULES_STATE_WAIT_FOR_MAP_TO_LOAD then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_WAIT_FOR_MAP_TO_LOAD")
	elseif new_state == DOTA_GAMERULES_STATE_PRE_GAME then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_PRE_GAME")
	...
	elseif new_state == DOTA_GAMERULES_STATE_GAME_IN_PROGRESS then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_GAME_IN_PROGRESS")
		bloodbound:OnGameInProgress()
	elseif new_state == DOTA_GAMERULES_STATE_POST_GAME then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_POST_GAME")
	elseif new_state == DOTA_GAMERULES_STATE_DISCONNECT then
		DebugPrint("[DEBUG] Game State changed to: DOTA_GAMERULES_STATE_DISCONNECT")
	end
end
